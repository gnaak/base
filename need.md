...
**남은 순서**: A4 → A6 → B7·B8 → 나머지

A1·A2·A3·A5 완료. 요청과 응답 양쪽에 타입이 붙었고, 로그인은 IP·계정 두 층으로 막혀 있다.
남은 보안은 A4(prod CSRF)와 A6(토큰 무효화 + `active` 미검사)다.
...
`chatbase.kr` 구현을 참고하되 네 가지를 고쳤다 (그쪽에도 역이식할 것):
